...
local function run()
  ...
  local function assert_false(value, label)
    assert_equal(value, false, label)
  end
  ...
  assert_false(switched_runtime_ok, "setup changed an already-selected runtime")
  ...
  assert_false(daemon_ok, "setup accepted a runtime without a live daemon")
  ...
  runtime.check_daemon = function()
    ...
    return false, "unexpected daemon check"
  end
  ...
  assert_equal(vim.api.nvim_get_current_line(), "  hello again world", "indented single word in spaces")
  ...
  assert_false(unexpected_ok, "unexpected load failure returned success")
  ...
  assert_true(not exists(max_id_path), "empty transcript was not removed")

  local invalid_id_notifications, invalid_id_ok, invalid_id_err = capture_notifications(function()
    return talk2text.load(max_transcript_id + 1)
  end)
  assert_false(invalid_id_ok, "unsafe transcript ID unexpectedly loaded")
  assert_true(
  ...
  assert_false(retry_ok, "missing transcript unexpectedly loaded")
  ...
  assert_false(blocked_ok, "unmodifiable buffer unexpectedly accepted a transcript")
  ...
  assert_equal(vim.api.nvim_get_current_line(), "third retried text blocked text", "retried blocked line")
  ...
  assert_false(bad_setup_ok, "unknown setup option was accepted")
  ...
  assert_false(missing_ok, "missing runtime setup unexpectedly succeeded")
  ...
  assert_false(cleanup_failure_ok, "cleanup registration failure returned success")
  assert_true(
  ...
  assert_false(failed_target_ok, "target registration failure returned success")
  ...
  assert_false(invalid_ok, "negative transcript ID unexpectedly loaded")
  ...
  local notifications = capture_notifications(function()
    talk2text.start_default_target(8)
  end)
